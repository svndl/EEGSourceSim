...
% [ProjectPath, AnatomyPath]  = ESSim.Simulate.PrepareProjectSimulate(ProjectPath,DestPath ,'FwdFormat','mat');
% 
% ProjectPath = '/Volumes/svndl/mrC_Projects/kohler/SYM_16GR/SOURCE';
% % This is to make a portable copy of the project data (both anatomy and forward)
% ESSim.Simulate.PrepareProjectSimulate(ProjectPath,DestPath ,'FwdFormat','mat');
% 
% 
...
% Pre-select ROIs
[RoiList,subIDs] = ESSim.Simulate.GetRoiClass(ProjectPath,AnatomyPath);% 13 subjects with Wang atlab 
...
[outSignal, FundFreq, SF]= ESSim.Simulate.ModelSeedSignal('signalType','SSVEP','signalFreq',[2 3],'signalHarmonic',{[2,0,1],[1,1,0]},'signalPhase',{[.1,0,.2],[0,.3,0]});
...
ESSim.Simulate.PlotEEG(MASDEEG_raw_comp,freq,[],'raw  condition 1',masterList1,fundFreqs);
%%
MASDEEG_raw_comp = mean(EEGAxx2{1}.Amp,3);
ESSim.Simulate.PlotEEG(MASDEEG_raw_comp,freq,[],'raw  condition 2',masterList2,fundFreqs);
%% PCA on full freq range
[PCAAxx,W,A,D] = ESSim.SpatialFilters.PCA(EEGAxx1{1},'freq_range',freq);
MASDEEG_pca_comp = mean(PCAAxx.Amp,3);
ESSim.Simulate.PlotEEG(MASDEEG_pca_comp,freq,[],'PCA on full freq. range ',masterList1,fundFreqs,[],[],[],[],[],A,W);

%% PCA on 1st stim freq
[PCAAxx,W,A,D] = ESSim.SpatialFilters.PCA(EEGAxx1{1},'freq_range',fundFreqs(1));
MASDEEG_pca_comp = mean(PCAAxx.Amp,3);
ESSim.Simulate.PlotEEG(MASDEEG_pca_comp,freq,[], sprintf('PCA on %0.1f Hz', fundFreqs(1)), masterList1,fundFreqs,[],[],[],[],[],A,W);

%% PCA on 2st stim freq
[PCAAxx,W,A,D] = ESSim.SpatialFilters.PCA(EEGAxx1{1},'freq_range',fundFreqs(2));
MASDEEG_pca_comp = mean(PCAAxx.Amp,3);
ESSim.Simulate.PlotEEG(MASDEEG_pca_comp,freq,[], sprintf('PCA on %0.1f Hz', fundFreqs(2)), masterList1,fundFreqs,[],[],[],[],[],A,W);

%% SSD on 1st stim freq
[SSDAxx,W,A,D] = ESSim.SpatialFilters.SSD(EEGAxx1{1},fundFreqs(1),'do_whitening',true);
MASDEEG_ssd_comp = mean(SSDAxx.Amp,3);
ESSim.Simulate.PlotEEG(MASDEEG_ssd_comp,freq,[],sprintf('SSD on %0.1f Hz', fundFreqs(1)),masterList1,fundFreqs,[],[],[],[],[],A,W);

%% SSD on 2nd stim freq
[SSDAxx,W,A,D] = ESSim.SpatialFilters.SSD(EEGAxx1{1},fundFreqs(2),'do_whitening',true);
MASDEEG_ssd_comp = mean(SSDAxx.Amp,3);
ESSim.Simulate.PlotEEG(MASDEEG_ssd_comp,freq,[],sprintf('SSD on %0.1f Hz', fundFreqs(2)),masterList1,fundFreqs,[],[],[],[],[],A,W);

%% CSP
[CSPAxx,W,A,D] = ESSim.SpatialFilters.CSP({EEGAxx1{1},EEGAxx2{1}},'freq_range',[2,3],'do_whitening',do_whitening);
class_vis = 1;
MASDEEG_csp_comp = mean(CSPAxx{class_vis}.Amp,3);
ESSim.Simulate.PlotEEG(MASDEEG_csp_comp,freq,[],'CSP on fundFreqs ',masterList1,fundFreqs,[],[],[],[],[],A,W);

%% CSP
[CSPAxx,W,A,D] = ESSim.SpatialFilters.CSP({EEGAxx1{1},EEGAxx2{1}},'do_whitening',do_whitening);
class_vis = 1;
MASDEEG_csp_comp = mean(CSPAxx{class_vis}.Amp,3);
ESSim.Simulate.PlotEEG(MASDEEG_csp_comp,freq,[],'CSP on all freqs ',masterList1,fundFreqs,[],[],[],[],[],A,W);

%% RCA
[RCAAxx,W,A,D] = ESSim.SpatialFilters.RCA(EEGAxx1{1},'freq_range',[2,3],'do_whitening',do_whitening);
MASDEEG_rca_comp = mean(RCAAxx.Amp,3);
ESSim.Simulate.PlotEEG(MASDEEG_rca_comp,freq,[],'RCA on fundFreqs ',masterList1,fundFreqs,[],[],[],[],[],A,W);

%% RCA
[RCAAxx,W,A,D] = ESSim.SpatialFilters.RCA(EEGAxx1{1},'freq_range',[2,3],'do_whitening',do_whitening, 'model_type','cartesian');
MASDEEG_rca_comp = mean(RCAAxx.Amp,3);
ESSim.Simulate.PlotEEG(MASDEEG_rca_comp,freq,[],'RCA on fundFreqs, indep. cplx comps. ',masterList1,fundFreqs,[],[],[],[],[],A,W);

%% RCA
[RCAAxx,W,A,D] = ESSim.SpatialFilters.RCA(EEGAxx1{1},'do_whitening',do_whitening);
MASDEEG_rca_comp = mean(RCAAxx.Amp,3);
ESSim.Simulate.PlotEEG(MASDEEG_rca_comp,freq,[],'RCA on all freqs ',masterList1,fundFreqs,[],[],[],[],[],A,W);
