function [p,S,mu] = polyfitBMroot(x,y,degree,b,m,root)
% POLYFITBM Fit polynomial to data, forcing y-intercept, slope and root
%   P = POLYFITBM(X,Y,N,B,M,ROOT) is similar POLYFIT(X,Y,N) except that the
...
p_root = [p_root;m_root;b_root];
p = conv(p_root,[1,-root]);
%% error estimates
% attribution: this is based on code from POLYFIT by The MathWorks Inc.
if nargout > 1
