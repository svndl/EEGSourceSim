function [p,S,mu] = polyfitBM(x,y,degree,b,m)
% POLYFITBM Fit polynomial to data, forcing y-intercept to zero.
%   P = POLYFITBM(X,Y,N,B,M) is similar POLYFIT(X,Y,N) except that the
...
p = z\(y-b-m.*x); % solve
p = [p;m;b]; % set y-intercept to zero
%% error estimates
% attribution: this is based on code from POLYFIT by The MathWorks Inc.
if nargout > 1
