function [p,S,mu] = polyfitBroot(x,y,degree,b,root)
% POLYFITB Fit polynomial to data, forcing y-intercept to zero.
%   P = POLYFITB(X,Y,N,B,ROOT) is similar POLYFIT(X,Y,N) except that the
...
b_root = -b/root;
y_root = y./(x-root) - b_root;
p_root = z\y_root; % solve
p_root = [p_root;b_root];
