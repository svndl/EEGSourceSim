function [reg_min,G,reg_param, minG] = gcv_MiMS(U,s,b,method) 
...
% SIAM, 1990. 
%
% Modified by Benoit Cottereau to dela with several time instants
%
% Set defaults. 
%if (nargin==3), method='Tikh'; end  % Default method. 
if (nargin==4), method='Tikh'; end  % Default method. 
...
beta2(find(beta2<0)) = 0;           % = 0 if the corresponding time sample don't lead to an intrinsic residual

%beta = reshape(beta(1:p,:),size(beta(1:p,:),1)*size(beta(1:p,:),2),1);   % Allows to deal with several time points
%beta2 = normb2 - norm(beta)^2; 
% if (ps==2) 
...
if (strncmp(method,'Tikh',4) | strncmp(method,'tikh',4)) 
  ...
  % Intrinsic residual. 
  delta0 = 0; 
  if (m > n & beta2 > 0), delta0 = sum(beta2); end 
   
  % Vector of GCV-function values. 
  for i=1:npoints 
    G(i) = gcvfun(reg_param(i),s2,beta,delta0,m-n,size(b,2)); 
  end  
...
%    
  ...
  if (find_min) 
    ...
    reg_min = fminbnd('gcvfun',... 
      ...
      optimset('Display','off'),s2,beta,delta0,m-n,size(b,2)); % Minimizer. 
    minG = gcvfun(reg_min,s2,beta,delta0,m-n,size(b,2)); % Minimum of GCV function. 
