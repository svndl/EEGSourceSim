...
x.delta = [0,0.38,0.5,0.54,1:0.5:3 50]; % in mm
x.theta = [0,0.38,0.5,0.54,1:0.5:3 50]; % in mm
x.alpha = [0,0.38,0.5,0.54,1:0.5:3 40]; % in mm
x.beta = [0,0.38,0.5,0.54,1:0.5:3 30]; % in mm
x.gamma = [0,0.38,0.5,0.54,1:0.5:3 10 20]; % in mm
y.delta = [7., 6.3, 6.1, 6.05, 5.5,  5.3,  5,    4.9,  4.8 0]/7; % strange number where read from print in cm
y.theta = [7., 6.1, 5.8, 5.7,  5.1,  4.8,  4.5,  4.4,  4.3 0]/7;
y.alpha =  [7., 6.3, 6.1, 6.05, 5.75, 5.5,  5.25, 5.1,  5 0]/7;
y.beta = [7., 5.1, 4.9, 4.8,  4.45, 4.2,  3.9,  3.8,  3.6 0]/7;
y.gamma = [7., 3.2, 2.9, 2.75, 2.3,  2.05, 1.9,  1.75, 1.6 .7 0]/7;
...
for i = 1:length(freq_band_names)
    ...
    for this_model_idx = 1:length(model_names)
        ...
        scatter(x.(freq_band_names{i}),y.(freq_band_names{i}),colors{i})
        hold on 
        if strcmp(this_model_name,'power_law')
             if strfind(version,'R2011')
             [this_p,R,J,CovB,MSE] =nlinfit(x.(freq_band_names{i})(1:end),y.(freq_band_names{i})(1:end),model_fun.(this_model_name),[1.,1.,1.   ,0.]);
             else
             [this_p,R,J,CovB,MSE,ErrorModelInfo] =nlinfit(x.(freq_band_names{i})(1:end),y.(freq_band_names{i})(1:end),model_fun.(this_model_name),[1.,1.,1.   ,0.]);
             end
        else
            if strfind(version,'R2011')
            [this_p,R,J,CovB,MSE] =nlinfit(x.(freq_band_names{i})(1:end),y.(freq_band_names{i})(1:end),model_fun.(this_model_name),[1.,1.,0  ,0.]);
            else
            [this_p,R,J,CovB,MSE,ErrorModelInfo] =nlinfit(x.(freq_band_names{i})(1:end),y.(freq_band_names{i})(1:end),model_fun.(this_model_name),[1.,1.,0  ,0.]);
            end
        end
...
save('spatial_decay_models_coherence_test','model_fun','model_params','best_model','band_freqs')
%%
% plot best model
if true
    close all
